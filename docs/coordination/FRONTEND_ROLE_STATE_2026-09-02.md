...
T: 2026-09-02 11:26:20 +08:00 (Asia/Taipei)
S: ACCEPTED / WAIT
...
X: current active role is DevOps Day 7. Frontend is WAIT.
...
K: `docs/reference/FRONTEND_M2_DAY6_PROGRESS_2026-09-01.md` — detailed Day 6 delivery history.
...
O1: no active frontend feature task.
...
N: Frontend = NONE / WAIT.
P: do not modify frontend UI, schema, fixture, evidence, or deployment while DevOps Day 7 is active.
X: next project task is DevOps Day 7 production hardening, not frontend implementation.
AC: DevOps must satisfy the acceptance criteria in `docs/coordination/ENGINEER_HANDOFF.md`, report evidence, then stop for Manager Day 8.
FE-REENTRY: only after an explicit Manager assignment. New frontend work must include a scoped acceptance criterion, contract dependency check, targeted test, typecheck/build/PWA check, and browser evidence when UI changes.

## 8. HANDOFF NOTES

H1: read first: `docs/coordination/ENGINEER_HANDOFF.md`; then `docs/PROJECT_HUB.md`; then only the role-specific files needed for the assigned task.
H2: for M2 UI changes, read `docs/coordination/FRONTEND_MANAGER.md`, `docs/coordination/DATA_ENGINEERING.md`, `docs/coordination/FRONTEND_BACKEND.md`, `docs/design/M2_FIGMA_DAY5_BUILD_SPEC.md`, and the evidence index before editing.
