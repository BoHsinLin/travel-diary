...
import { buildSeoulImportPlan, normalizedPlaceName, reviewRiskFlags, validateSeoulDataset } from './seoul-content-import.js';
...
function dataset(overrides = {}) {
  const places = Array.from({ length: 120 }, (_, index) => place(index + 1, index < 12 ? 'official' : index < 57 ? 'verified' : 'unverified', index < 43 ? 'https://english.visitseoul.net/place' : null));
...
test('plan prioritizes traceable official and verified rows and quarantines missing sources', () => {
  ...
  assert.deepEqual(plan.counts, { total: 120, accepted: 43, quarantined: 77, official: 12, verified: 45, unverified: 63, acceptedOfficial: 12, acceptedVerified: 31, acceptedUnverified: 0 });
