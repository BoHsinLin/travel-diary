...
import { acknowledgeChangeNotification, listChangeNotifications, sourceChangeNoticesKey } from './changeNotificationsRepository';

export function ChangeNotifications({ tripId }: { tripId?: string }) {
  const queryClient = useQueryClient();
  const [failedId, setFailedId] = useState<string | null>(null);
  const [savingId, setSavingId] = useState<string | null>(null);
  const notices = useQuery({ queryKey: sourceChangeNoticesKey(tripId), enabled: Boolean(tripId && supabase), queryFn: () => listChangeNotifications(tripId!) });
  if (!notices.data?.length) return null;
  const acknowledge = async (id: string) => {
    setSavingId(id); setFailedId(null);
    try {
      await acknowledgeChangeNotification(id, new Date().toISOString());
      await queryClient.invalidateQueries({ queryKey: sourceChangeNoticesKey(tripId) });
